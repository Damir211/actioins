...
if (document.getElementById('phone-mask')) {
  var documentPhoneInput = document.getElementById('phone-mask');
  var documentPhoneInputContainer = document.querySelector('.actionform__tel');
  var phoneMask = IMask(documentPhoneInput, {
    mask: '+{7} (000) 000 00-00'
  });
  documentPhoneInput.addEventListener('input', function () {
    if (this.value.length === 18) {
      documentPhoneInputContainer.classList.add('active');
    } else {
      documentPhoneInputContainer.classList.remove('active');
    }
  });
...
var documentInput = document.querySelector('.actionform__doc');
var documentInputText = document.querySelector('.actionform__doc-text');
...
if (documentInput) {
  documentInput.addEventListener('change', function (e) {
    ...
    documentInputText.textContent = fileName;
  });
}

function startEndTimer(arrayEndDates) {
  var countDownDate;
  var currentDate = new Date().getTime();
  var dateIsFinded = false;
  arrayEndDates.forEach(function (date) {
    if (dateIsFinded) {
      return;
    }
    ...
    if (countDownDate - currentDate < 0) {
      return;
    } else {
      dateIsFinded = true;
    }
  ...
  var x = setInterval(function () {
    ...
    document.getElementById("timer").innerHTML = days + " дней " + hours + " часов " + minutes + " минут"; // If the count down is over, write some text

    if (distance < 0) {
      clearInterval(x);
      document.getElementById("demo").innerHTML = "00 дней 00 часов 00 минут";
    }
  }, 1000);
}